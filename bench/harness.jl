...
function print_table(results::BenchmarkGroup, meta::Dict{String,WorkloadMeta}; io::IO=stdout)
    ...
    data = Matrix{Any}(undef, length(names), 9)
    ...
    pretty_table(io, data;
                 column_labels=["workload", "mode", "min", "allocs",
                                "primal", "primal allocs", "×", "comms B", "isbits"],
                 alignment=[:l, :l, :r, :r, :r, :r, :r, :r, :r],
                 highlighters=[hl_alloc, hl_palloc, hl_ptr],
...
# machine any workload can swing further, and the fix is to raise this, not to ignore the flag.
function print_comparison(before::BenchmarkGroup, after::BenchmarkGroup,
    ...
    pretty_table(io, data;
                 ...
                 highlighters=[hl_slower, hl_faster, hl_noise],
                 display_size=(-1, -1))
...
# established IR-based AD tool on the same primals — so there's no noise tolerance, just the two
# numbers and their ratio. Iterates `keys(enzyme_results)` rather than every Differ key: only the
# core subset has an Enzyme twin.
function print_vs_enzyme(differ_results::BenchmarkGroup, enzyme_results::BenchmarkGroup,
    ...
    pretty_table(io, data;
                 column_labels=["workload", "mode", "Differ min", "Differ allocs",
                               "Enzyme min", "Enzyme allocs", "Differ/Enzyme"],
