...
# calls into (`contextual.jl`), and the split-shadow dualization engine itself.
#
...
# and invokes the resulting `CodeInstance`. The dualization is a split-shadow transform on `g`'s
...
# callee or an argument was too abstractly typed to wrap statically). Rebuilds concrete `Dual`s from
# the runtime primal/tangent values: `map(Dual, ...)` is the trick — `Dual(1.0, 1.0)` infers the
# concrete `Dual{Float64,Float64}`, letting `frule!!`'s `@generated` dispatch (keyed on the `Dual`
# type parameters) name a concrete primal method, which a statically-built `Dual{Any,Any}` couldn't.
# The callee carries its real tangent `tf`, not a forced zero, so a dynamically-dispatched closure's
...
# mode is loaded.
#
# Argument deliberately left untyped (not `::MethodInstance`) in every default hook in this
# section: an override living in a package extension must be a strictly *more specific* method
# than the default, or Julia treats it as an illegal same-signature overwrite across modules
# ("Method overwriting is not permitted during Module precompilation" — hit empirically writing
# `DifferForwardsOverReverseExt`). Typing the argument concretely here would make
# `DifferForwardsOverReverseExt`'s `mi::MethodInstance` override collide instead of add.
...
# genuine build failure (propagated), not "not applicable" — see `_is_foreign_mode_carrier`.
...
# forward-over-reverse when the reverse-mode carrier's own constructor/field-access calls get
# inlined into raw `%new`/`getfield`/`setfield!` before this dualizer ever sees them)? Default
# `false`.
_foreign_selfsim_shadow_type(@nospecialize(T)) = false

# The shadow's field type at index `fi` of a foreign self-similar-shadow type `T`, when it's
# sound to mirror `getfield`/`setfield!` directly, or `nothing` when the field carries no
# tangent (in which case the primal value is carried through instead — see `builtins.jl`). Only
# called when `_foreign_selfsim_shadow_type(T)` is `true`. The logic itself is fully generic
# (works for any type whose own `tangent_type` maps fields through `fieldtype` directly, not
# specifically DifferReverse's types), so unlike the other coupling-point hooks this default
# implementation doesn't need overriding — only the `_foreign_selfsim_shadow_type` gate above
# does.
...
# in lockstep between primal and shadow on a `setfield!` write — bookkeeping, not differentiable
# content (e.g. DifferReverse's `Stack.position`) — rather than left untouched. Only consulted
# when `_foreign_selfsim_shadow_field` returned `nothing` for that field.
...
function build_contextual_ir(interp::ContextualInterpreter{Forward}, mi::MethodInstance)
    ...
    # not-yet-written `frule!!`) later appears, the mt-backedges recorded below (see
...
# which ordinary inference produces), and `.parameters` on a `UnionAll` throws an uncaught
...
# Build a minimal IRCode that just `error(msg)`s when invoked, installed via the same
# `finishinfer!`/`optimize` path as a real dualized body (transform IRCode only, never patch a
# rettype after the fact). Used when `build_dual_ir` bails, so calling the carrier reports why
# (the specific unsupported construct) instead of `dualized_impl`'s generic stub message.
...
function implicit_frule_tt(callee_mi::MethodInstance)
    ...
    try
        ...
        # giving up: `findsup` resolves such a query fine (a hand rule if one exists, else the
        # generated fallback), and a hand-written rule for a vararg function is exactly the case that
        # needs this. An imprecise match only ever restricts: `has_hand_frule` suppresses an inline,
...
# went through `frule_split!`/`frule_codeinstance` — see the call site in `build_dual_ir`'s base case.
# So a user later hand-writing `frule!!(::Dual{typeof(callee_mi's function)}, ...)` invalidates a
# derivative built before that rule existed, same as a surviving high-level call would get via
...
# below to keep such a call from being inlined away before `dualize_to_ircode` gets a chance to route
...
# vanishes into the caller's optimized IR (as plain arithmetic or whatever it lowers to) before
# `dualize_to_ircode` ever sees it, so the hand rule can never be consulted, regardless of how
# small/inlinable Julia's ordinary cost-based heuristic (which has no concept of `frule!!`) thinks the
# callee is. Falls back to the ordinary policy otherwise, so this only ever restricts inlining
# relative to normal Julia behavior, never expands it.
...
# to surface to the user instead of a generic message. `edges` collects the backedges this resolution
# depends on: an unconditional mt-backedge on `primal_tt` (any new method that could apply to these
# argument types, more specific or previously nonexistent, must invalidate a derivative built against
# the old resolution), plus, when a match is found, a direct edge to `primal_mi` itself (this dual IR
# is built directly from its optimized IR, effectively inlining it, so redefining/invalidating it
# must invalidate us).
function primal_of_impl(interp::ContextualInterpreter, impl_mi::MethodInstance, reason::Ref{String}=Ref(""),
    ...
    # flat, exact list of `Dual`s the call supplied — no arity ceiling. A guard rather than an assert,
    # in case a carrier `MethodInstance` is ever reached by some other route.
...
# `ContextualInterpreter{Forward}` to resolve the inner `dualized_impl`), so a guard scoped to one interpreter
...
#  * Higher order (compose the transform): a request whose value args are nested Duals (e.g.
#    `Dual{Dual{F,F},Dual{F,F}}`) is differentiating the order-(k-1) dualized function. That
...
function _build_dual_ir(interp::ContextualInterpreter, impl_mi::MethodInstance, reason::Ref{String}=Ref(""),
    ...
    function compose(offset::Int)
        ...
        # instructions are copied/re-dualized wholesale below), so redefining/invalidating `inner_mi`
        # must invalidate this one too. `inner_mi`'s own dependencies (its primal method, any
        # `frule!!`s it resolved) are tracked on its own edges list when it's compiled via
        # `build_contextual_ir` — not duplicated into `edges` here.
    ...
    #
    #  * Uniform seeds (`code_dual_ircode` order≥2 / a `frule!!(fseed_k, seed_k)` call): every dual
    #    arg, function included, is nested one level, so an order-≥2 request is one where any
    #    argument's primal is itself a `Dual`. The function nests as arg 1, so the whole list peels
    #    (`offset=0`). A `frule!!` slot is excluded: `frule!!` is the only Dual-consuming function, so
    #    a Dual-valued arg under `frule!!` means "differentiate a hand rule once" (the base case), not
    ...
    #    dualizes a function whose body called `frule!!`, that inner call survives in the primal IR as
    #    a `dualized_impl` `:invoke` (inlined generated `frule!!`) or a `frule!!` `:invoke`.
    #    `frule_split!` re-wrapped its callee as the non-nested function slot
    #    `Dual{typeof(dualized_impl),NoTangent}` / `Dual{typeof(frule!!),NoTangent}` at position 1,
    #    naming the function being re-differentiated rather than a value arg — so it's dropped
    #    (`offset=1`) and the trailing nested args peel.
    ...
    # resolves to the generated (vararg) `frule!!` rather than a hand rule, this isn't a base case but
    # a composed derivative (`D` applied to a surviving `frule!!` invoke, e.g. a 3rd+-order nested
    ...
    # Forward-over-reverse: `primal_mi` can itself be a carrier owned by a different AD-mode
    # package (e.g. DifferReverse's `reverse_fwds_impl`/`reverse_pullback_impl`) rather than an
    # ordinary user method — reached when forward-differentiating a `rev_gradient`/
    # `value_and_gradient!`/`Tape` pullback call. Dualizing such a call surfaces `rrule!!`'s
    # generated body, whose one real statement is an `invoke` against an already-compiled
    # `CodeInstance`; re-dualizing that surviving invoke is what lands here with `primal_mi`
    # resolved to the carrier stub method. Ordinary `CC.typeinf_frame` under
    # `ContextualInterpreter{Forward}` would just recompile the untransformed stub, since only
    # that other package's own interpreter recognizes these mi's — so `_foreign_mode_primal_ir`
...
# (and what `code_dual_ircode` returns) — `build_dual_ir` followed by the IPO-safe passes. Used both
# by the higher-order recursion above (to obtain the order-(k-1) dual IR as a primal) and by the
...
# `typeinf_ext_toplevel(NativeInterpreter, …, SOURCE_MODE_ABI)`) and identical invalidation edges — an
# mt-backedge so a new/more-specific method invalidates this carrier, plus a direct invoke edge to
# the resolved CI. Used for the tangent-helper calls (`get_tangent_field`/`set_tangent_field!`
# fallbacks, `zero_tangent`) the direct-emission path can't take: a synthesized bare
...
# `IRCode` into a dualized `IRCode` using the "split-shadow" scheme: the primal computation is
# reconstructed and a parallel tangent computation is emitted beside it, then packed into a `Dual`.
# Low-level rules describe how each intrinsic (`add_float`, `mul_float`, …), builtin (`getfield`),
# and `%new` propagates tangents; surviving `:invoke`/`:call`s (e.g. `sin`/`cos`) go through
# `frule!!` dispatch, which picks up hand-written rules.
#
# Types are derived directly from the primal IR, not guessed: every shadow (tangent) statement
# shares its primal statement's type `Ti`; a `Dual{R,R}` wrapper uses `R = Ti`; a surviving `frule!!`
# result is `Dual{R,R}` with `R` the primal call's result type. The result is a fully typed `IRCode`
# that installs as the optimization result, whose return type `finishinfer!` reads off via
# `compute_ir_rettype` — no re-inference. Branches and loops (`GotoNode`/`GotoIfNot`/`PhiNode`) are
# supported: block topology is preserved 1:1 from the primal, so only within-block instruction counts
# change. Throwing error paths are supported too: a block terminating in an unreachable `ReturnNode`
# (a domain/bounds check's throw target) is reconstructed primal-only, so the derivative reproduces
# the same throw — see `unreachable_block` below. Exception handling (`try`/`catch`) is supported as
# well: `UpsilonNode`/`PhiCNode` are duplicated into primal + shadow copies and
# `EnterNode`/`:leave`/`:pop_exception` carry over as control markers. Returns `nothing` to bail on
# still-unsupported constructs — e.g. a `Core.Builtin` with no rule (`Core.memoryrefoffset`, used by
# `push!`/`resize!`; a non-bits/undef-checked element access; or the `Core._apply_iterate` left
# behind by splatting something whose length isn't statically known) — and the caller bails
# gracefully. Vararg primal methods are supported (see `primal_nfixed` below); only a splat call site
# over a non-tuple is not. Array element read/write, array allocation
# (`zeros`/`similar`/`Vector{T}(undef,n)`/comprehensions, via `memorynew`), and mutable-struct field
# mutation (`setfield!`) are supported — see the
# `memorynew`/`memoryrefnew`/`memoryrefget`/`memoryrefset!`/`setfield!` arms below.
# ===========================================================================
...
# (`f(a, b, xs...)`) — the number of slots it declares before the vararg one (`#self#` included, i.e.
# `method.nargs - 1`). The prologue then re-packs the trailing flat dual args into the single tuple
# slot `pir` expects at `Argument(primal_nfixed+1)`, after which the rest of this transform sees
# exactly the argument shape `pir` was compiled with and needs no vararg awareness. Only meaningful
# with `pir_is_vararg=false`: an order-(k-1) dual carrier has already absorbed any vararg-ness of its
# own primal into its flat `argtypes`.
...
function dualize_to_ircode(interp, impl_mi::MethodInstance, pir, n::Int;
    ...
    # unconditional arg-extraction prologue below, which must land before any real phi. Not observed
    # in practice (slot2ssa! always keeps block 1 phi-free), but bail rather than emit invalid IR if
    # it ever occurs.
    ...
    # non-Core/Base module: `Core.Compiler.verify_ir` rejects a bare `GlobalRef` used directly as a
    # value unless its binding is proven constant across the IR's valid worlds, which these
    ...
    # (`Float64`, `Float32`, `Complex`, …) `tt(T) == T`, so scalar shadows are unchanged from the old
    # same-typed scheme; for general structs it's a `Tangent`/`MutableTangent`, for tuples a per-field
    # tangent tuple, and for a `Dual` carrier the `Dual` itself (see `dual.jl`).
    ...
    # a resolved call runs the compiled method directly instead of dynamic-dispatching a
    # `CallInfo`-less synthesized `:call`. `argtypes` are the argument types (excluding `f`), used
    # only to build the dispatch tuple — pass the operands' own declared types so the resolved
    # `CodeInstance` matches what runs.
    #
    ...
    # dispatches correctly on the concrete runtime value.
    ...
    # `Type{P}`-typed callee/operand always takes the dynamic (`dynamic_frule`) path below rather than
    # a static `:invoke` — correct, just not the fast path.
    ...
    # a dynamic dispatch). A `%new(Dual{R,tt(R)}, …)` would freeze the over-wide declared type into
    # the value — `dynbox(3.0)` would return a boxed `Dual{Any,Any}(9.0, 6.0)` instead of a genuine
    # `Dual{Float64,Float64}`, which couldn't flow back into `frule!!`. Instead call the `Dual`
    ...
    # `Dual{Union{…},…}` is not a subtype of that union, so `%new` can't be widened this way).
    ...
    # resolve — `_globalref_val`'s reason), and asks constness at `iworld`, never the ambient world
    # (`_globalref_isconst`'s reason).
    ...
    # `verify_ir` unless its module is `Core`/`Base` or its binding is proven constant across the IR's
    # valid worlds (`check_op` in `Compiler/src/ssair/verify.jl`) — see gotcha #2 in the
    ...
    # unresolvable, leaving the caller's `isa(T, Type)` check to catch it.
    ...
    # a literal so no call survives into the IR. `zero_tangent` handles singletons (-> `NoTangent()`
    # for a function/`Int`), scalars (`0.0`), `Dual` carriers (a same-typed zero), and composite
    # constants (a `Tangent`/tuple of zeros).
    #
    ...
    # genuine runtime `zero_tangent` call on the re-embedded, raw `GlobalRef` operand — never splice a
    # constructed tangent object as a compile-time literal: a mutable tangent (e.g. a
    ...
    function const_tangent(@nospecialize x)
        ...
        # (`tangent_type(Ptr{NoTangent}) === Ptr{NoTangent}`) — which keeps IR containing it
    ...
    # `tt`'s note above) — widen it first, same as `tt`, so the `Number` literal-zero fast path and
    # the `emit_invoke!` argtype (which needs a real `Type` for its dispatch tuple) both still apply
    # instead of silently falling back to a plain dynamic `:call`.
    ...
    if !pir_is_vararg
        ...
        if primal_nfixed !== nothing
            ...
            # legitimately sharper than inference's summary of the same slot (`Core.Const((Float64,))`
            ...
            # against. A genuine disagreement (a different arity) still fails. This bails rather than
            # asserting, unlike the `pir_is_vararg` branch below where both sides are this pass's own
            # construction: here the shape is driven by user argument types, and an `AssertionError`
            # out of the `@generated frule!!` body aborts compilation instead of producing an
        ...
        # Dual`); peel one level off each: reconstruct a primal tuple (each element the order-(k-1)
        # dual value) and a tangent tuple (its new, k-th tangent direction). The reconstructed tuple
        # must match `pir`'s own vararg-tuple arg type, so the tuple index `i` is the original
        # dualparam position (offset-shifted) while the tuple element position `j` is offset-free.
        # `pir` has exactly two arguments (#self# and the tuple), so `parg`/`targ` are length 2
        # regardless of `n`.
    ...
    # `presolve` for an operand that lands in value position (a `%new` field). Plain `presolve`
    # passes a `GlobalRef` through unchanged, which `verify_ir` rejects — resolve it via
    # `gref_operand!`. Call/invoke argument positions don't need this: `frule_split!` and the
    # `Core.Const` arm already embed a statically-known operand's resolved value via `_calleeval`.
    ...
    # directly, for three reasons. (1) `pir.argtypes` holds lattice elements, not necessarily bare
    ...
    # would report `GlobalRef`, the type of the node, not of the value the binding names. That's a
    # silent miscompile, not a bail: `getfield(Main.CONST_VEC, :ref)` would take the builtin rule's
    # general-struct branch (`GlobalRef` isn't `<: Array`) and emit `get_tangent_field` against an
    # array shadow, `MethodError`ing at run time. Take the type from the binding instead, exactly as
    # the `ReturnNode` arm does. `_optype` still handles `SSAValue`s and literals, and stays as-is for
    # reverse mode's own call sites.
    ...
    function frule_split!(fpos, actual, R)
        ...
        # `frule!!`'s `@generated` dispatch, keyed on exactly those `Dual` type parameters, couldn't
        ...
        # (typed `tt(R)`, which widens to `Any` when `R` is abstract). We do not branch on `R`: a call
        # with concrete callee+args but an inference-widened abstract result (e.g. `_2 * _2 :: Any`,
        # left after a `Ref{Any}` was SROA'd away) takes the static path below, which annotates its
        # result with the abstract `dual_type(R)` — sound because `Dual` is invariant, so the concrete
        # `Dual{Rc,Tc}` the rule returns is `<: Dual` but not `<: Dual{Any,Any}`.
        ...
        # `frule!!` dispatch to resolve. Only genuinely dynamic operands (SSAValue/Argument) fall back
        # to `presolve`/`_optype`/`tresolve`.
        ...
        # we can resolve one (direct, unboxed call); otherwise a plain non-inlined `:call`.
        # `dual_type(R)` is the declared result type: `Dual{R,tangent_type(R)}` for a concrete `R`,
        ...
        dd = if callee_impl_mi !== nothing && callee_impl_mi === impl_mi
            ...
            # `cache_owner(::ContextualInterpreter{Forward}) === Forward()`, so that would silently run
    ...
    # machinery — see that file for why intrinsics get this cheaper path. `optype`/`tt` give the
    # pointer rules (`bitcast`/`pointerref`/`pointerset`/`add_ptr`) the same primal-type introspection
    # the builtin rules get; `reason` lets a rule that declines say why (see the dispatch sites below).
    ...
    # transform only expands each original statement into more instructions, never splits, merges, or
    # reorders blocks. So `GotoNode.label`/`GotoIfNot.dest`/`PhiNode.edges` (already basic-block
    ...
    # the primal computation is rebuilt faithfully so the derivative raises the same error on the same
    # inputs, but no shadow/tangent is computed for it.
    ...
    # walk (a loop back-edge) registers a `pending` forward-reference patch instead.
    ...
    for i in 1:N
        ...
        if unreachable_block[bidx]
            # Primal-only reconstruction (see `unreachable_block` above). `shadow[i] = primal[i]` is a
            # never-consumed placeholder so `presolve`/`tresolve` on later error-path operands still
            # resolve, without emitting a bogus tangent computation.
            ...
            elseif isa(s, Expr) && s.head === :new
                ...
                # live-path `:new` arm below (ISSUES #60) — a throw block builds exception objects, and
            ...
            elseif isa(s, Expr) && s.head === :throw_undef_if_not
                ...
                # `args[2]` is the non-differentiable Bool condition, which needs this pass's own
        ...
        elseif isa(Ti, Core.Const) && isa(s, Expr) && (s.head === :call || s.head === :invoke)
            ...
            # quantity is definitionally zero, independent of what the callee actually computes.
            ...
            # shadow.) The call is kept in the primal IR only for its own sake (effects, or simply not
            # inlined), so still reconstruct it faithfully — but skip the intrinsic/builtin/`frule!!`
            # dispatch below entirely for the shadow, straight to `zero_shadow`. Resolve the
            # display-callee to its value exactly like the unreachable-block arm above (a bare
            # non-Core/Base `GlobalRef` in value position is rejected by `verify_ir`).
        ...
        elseif isa(s, Core.ReturnNode)
            if !isdefined(s, :val)
                ...
                if isa(s.val, GlobalRef)
                    ...
                    # from the binding instead. A defined `const` binding gives a concrete
                    # `Core.Typeof(gv)` — for `return nothing` that's `Nothing`, keeping the
                    # allocation-free `%new` path; a non-`const` (or unresolvable) binding falls to
                    # `Any` and the dynamic pack.
                    gr = s.val::GlobalRef
                    p = gref_operand!(gr)
                    R = gref_optype(gr)
                else
        ...
        elseif isa(s, Expr) && s.head === :new
            ...
            # which throw a raw `TypeError` on a `GlobalRef`, and it's an operand position `verify_ir`
            # checks too (ISSUES #60). Resolution alone here, without the `const` test
            # `gref_constval` applies to field operands: a struct name is always a constant binding,
            # and refusing one a world-lookup happened to report otherwise would turn a working case
            # into a bail. `isa(gv, Type)` is the actual guard.
            T = resolve_new_type(s.args[1])
            if !isa(T, Type)
            ...
            elseif _foreign_selfsim_shadow_type(T)
                ...
                # inlined into raw `%new`s before this dualizer ever sees them. Same shape as the
                # `Dual` arm above, generalized per field via the `_foreign_selfsim_shadow_field`
                # hook — the same predicate `getfield`/`setfield!` use (`builtins.jl`), so a
        ...
        elseif isa(s, Expr) && s.head === :boundscheck
            ...
            # a 3-arg `getfield`/`memoryref*` boundscheck argument. Emitted through unchanged; its own
            # result is a non-differentiable `Bool`.
            primal[i] = emit!(Expr(:boundscheck, (presolve(a) for a in s.args)...), Ti)
            shadow[i] = zero_shadow(Ti, primal[i])
        elseif isa(s, Expr) && s.head === :throw_undef_if_not
            ...
            # copied through verbatim, never presolved/dualized. `args[2]` is the Bool condition (a
            # literal in a throw-only block, or a genuine SSA operand on a live path) and needs
        ...
        elseif isa(s, Core.UpsilonNode)
            ...
            # into a primal + shadow upsilon, exactly like a `PhiNode`. An unassigned `ϒ ()` slot (a
            # live-but-undefined capture) is mirrored as an empty upsilon in both copies.
            if isdefined(s, :val)
        ...
        elseif isa(s, Expr) && s.head === :foreigncall
            ...
            # write through any pointer it's handed, so unlike an unregistered intrinsic there's no
        ...
        elseif isa(s, Expr) && s.head === :loopinfo
            ...
            #
            # Two invariants this relies on: codegen consumes the marker positionally from the block's
            # terminator backwards, so nothing may be emitted between it and the terminator (this pass
            # emits shadow statements before it, which is fine); and a `julia.ivdep` marker carries
            # over to the dualized loop, where it now also asserts non-aliasing for the shadow buffer's
            # accesses — true whenever it was true of the primal, since the shadow mirrors the
        ...
        elseif isa(s, Expr) && s.head === :gc_preserve_begin
            ...
            # would let the shadow array be collected while a shadow `Ptr` into it is still live. A
            # tangent that isn't an SSA/argument (a literal `NoTangent()`, a spliced constant tangent)
            # is skipped: there's no heap object of ours to root. Type is `Any`, matching what
            # inference assigns the token.
            pargs = Any[]
            for a in s.args
        ...
        elseif isa(s, GlobalRef)
            ...
            # it must be emitted as a real instruction. Aliasing it away would let the raw `GlobalRef`
            # leak into a later operand position, which `Core.Compiler.verify_ir` rejects unless the
            # binding is proven constant (a non-`const` global like a plain `Ref{Any}` isn't). The
            # loaded value is external, non-differentiable state, so its shadow is the zero tangent
            # (matching how other non-diff sources are treated).
            primal[i] = emit!(s, Ti)
            shadow[i] = zero_shadow(Ti, primal[i])
        elseif !isa(s, Expr)
...
# `_optype`/`_optype_w`/`_stmt_str` (type-of-operand and bail-message helpers) now live in
# `DifferCore/src/shared_ir_helpers.jl`, shared with `DifferReverse`'s `_static_recursible_call`
# and friends.

# A field's shadow carries the primal value through, rather than its tangent, exactly when the
# field's tangent is `NoTangent` but its slot type would reject a `NoTangent` value — the field is
# non-differentiable yet its slot isn't itself a `NoTangent` slot. `tangent_type(T) === NoTangent` is
# the principled statement of that (it subsumes the old singleton-only test: every non-`NoTangent`
...
#    differentiable), so conservatively leave it on the tangent path.
#  * `T <: Type` handled separately — `Type{Float64}`/`DataType` are `Type`-valued (tangent
#    `NoTangent`) but not concrete (`isconcretetype(Type{Float64}) === false`), and
#    `Base.issingletontype` reports them non-singleton (a documented Julia quirk), so the concrete
...
function frule_body(world::UInt, source, self, dual_argtypes)
    ...
    # `impl_mi` is a real backedge: if its own CodeInstance is invalidated (that's where the
    # primal/`frule!!` dependencies actually get registered, via `finishinfer!` in `contextual.jl`),
