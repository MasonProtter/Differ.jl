...
# Nest a value's dual seed `order` levels deep: order 1 → `Dual{T,tangent_type(T)}`. Because a
# `Dual` is its own tangent type, the recursive step collapses to `Dual{S,S}`, so order 2 →
# `Dual{Dual{T,U},Dual{T,U}}`, etc. — matching Differ's Option-A higher-order nesting.
#
# Uniform for the function and every value argument (the function nests like any other value — see
# the peel in `build_dual_ir`). A plain function has `tangent_type(typeof(f)) == NoTangent`; a
# closure has a `Tangent{…}` tangent over its captures.
