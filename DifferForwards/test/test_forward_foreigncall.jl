...
@testset "second order (the dualized memmove is re-dualizable)" begin
    ...
    @test DifferForwards.primal(DifferForwards.primal(r)) ≈ 4.0     # f(x)     = x[1]^2
    @test DifferForwards.tangent(DifferForwards.primal(r)) ≈ 4.0    # f'(x)    = 2x[1]
    @test DifferForwards.tangent(DifferForwards.tangent(r)) ≈ 2.0   # f''(x)   = 2
