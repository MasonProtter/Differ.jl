...
using DifferForwards: Dual, NoTangent, frule!!, build_tangent, zero_tangent, unit_tangent, code_dual_ircode
...
@testset "user function with a hand-written frule!! (world-age callee resolution)" begin
    ...
    call_lin(x) = hr_lin(x)
    call_sqr(x) = hr_sqr(x)
    DifferForwards.frule!!(::Dual{typeof(hr_lin)}, d::Dual) = Dual(d.x + 1, d.dx * one(d.x))
    DifferForwards.frule!!(::Dual{typeof(hr_sqr)}, d::Dual) = Dual(d.x * d.x, d.dx * (2 * d.x))
