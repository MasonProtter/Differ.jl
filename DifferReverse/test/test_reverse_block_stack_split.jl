...
using DifferReverse: ID, CFGBlock, IDGotoIfNot, IDGotoNode, IDPhiNode, new_inst,
...
using DifferReverse: CoDual, NoRData, rrule!!, build_ctx, zero_fcodual, Ctx
...
@testset "memloop!: block-stack traffic is flat" begin
    ...
    function run_memloop(N)
        ...
        @test all(s -> !(s isa DifferReverse.Stack) || s.position == 0, pb.comms)
