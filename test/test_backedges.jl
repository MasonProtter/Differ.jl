# Tests that ADNext attaches real Julia backedges to a compiled derivative, so redefining a primal
...
# `primal_of_impl`/`frule_codeinstance`/`compose`): while dualizing a primal's IR, ADNext collects
...
@testset "backedges: derivative invalidation" begin
    ...
    @testset "a hand rule for an inlinable callee is honored, not inlined away" begin
        ...
        function ADNext.frule(::Dual{typeof(inlinable_callee)}, (; x, dx)::Dual)
    ...
    @testset "a hand rule added after first use still invalidates + is honored" begin
        ...
        function ADNext.frule(::Dual{typeof(inlinable_callee2)}, (; x, dx)::Dual)
            Dual(x + 10, 10 * dx * one(x))
        end
