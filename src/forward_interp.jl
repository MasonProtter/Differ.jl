# Forward-mode AD: the `frule!!` entry point, the `build_contextual_ir` override `ADInterpreter`
...
# hand-written rule, it compiles a dualized version of `g`'s body under `ADInterpreter{Forward}`
...
# Resolve a callee-position AST node to its actual value when it's statically known (a `GlobalRef` to
# a defined binding, or a `QuoteNode` literal). An `SSAValue`/`Argument` is a genuinely dynamic value,
# not a name to look up: returning it as-is would let the raw old-numbered node leak directly into the
# freshly built IR as an operand, referencing the wrong slot there. `nothing` signals "not statically
# resolvable — resolve via `presolve`/`_optype` instead" to callers.
#
# A `GlobalRef` is resolved at `world` — the interpreter's inference world — via `Base.getglobalref`
# (`jl_eval_globalref`), NOT the ambient task world. This is load-bearing: `dualize_to_ircode` runs
...
# (or `invoke_in_world`, which reparameterizes dispatch but not global-binding lookup) would there see
# a genuinely-defined user function as undefined, returning `nothing` — the callee then degrades to a
# raw `GlobalRef` and miscompiles at higher order (a `Dual{GlobalRef,…}` -> `%new` `TypeError`).
# `Base` functions escape this only because they're defined at an early world. The `world` argument is
# therefore mandatory (no default): every caller must thread the inference world in consciously — see
# the "world-age inside the generated `frule!!` body" note in the `differ-ircode-dualization` skill.
...
# `nothing` (exactly `return nothing`, which survives as `Main.nothing`). Returns `(true, val)` for a
# defined binding, `(false, nothing)` otherwise. Same world-parameterized `Base.getglobalref` lookup
# as `_calleeval` (see its note): `world` is mandatory, never the ambient task world.
...
# declaration, so it reports `false` for a genuinely constant binding (measured, not theoretical — a
# `const` global array operand degraded to `Any` and took the wrong branch in the `getfield` rule).
# Same check `verify_ir` performs on a value-position `GlobalRef`
...
#
#  * a `GlobalRef` names a binding rather than being a value, so `_optype`'s literal fallback answers
#    `GlobalRef`, the type of the node. Resolve it the way `gref_optype` does (const-gated at
#    `world`, never the ambient task world — see `_globalref_isconst`); a non-`const` binding is only
#    knowable at run time, hence `Any`. (In practice a non-`const` binding survives optimization as
#    its own `%k = Main.g` load statement, so it arrives here as an `SSAValue`.)
...
#    primal's own const-prop produced; every consumer here wants the widened `Type` (`_widen`,
#    defined with the rest of the reverse-mode type helpers in `reverse_interp.jl`).
#
