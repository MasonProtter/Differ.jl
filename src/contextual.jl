...
# AD modes.
#
# `ADInterpreter` is parameterized by the AD mode it runs in, so one interpreter and one set of
# `finishinfer!`/`optimize` overrides serve every mode; only the IR transform is mode-specific.
# Forward mode is implemented in `forward_interp.jl`, reverse mode in `reverse_interp.jl`.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ADInterpreter: a custom AbstractInterpreter that compiles a *contextually transformed* version of a
# primal method. The transform is a single source-to-source pass on the primal's post-optimization
# `IRCode`, installed into the normal typeinf pipeline so it produces an ordinary `invoke`-able
# `CodeInstance`. The concrete transform is chosen by the mode parameter `M` (e.g. forward-mode
# dualization in `forward_interp.jl`). Modeled on the plugin shape in
...
struct ADInterpreter{M<:ADMode} <: AbstractInterpreter
    ...
    # these into `me.src.edges` so the ordinary `compute_edges!`/`store_backedges` path registers real
    # Julia backedges, without Differ calling any invalidation ccall itself.
    ...
    # Keyed by the *carrier* mi, not the primal mi: the fwds carrier has two independent
    ...
    # sibling — a bounded, one-off nested compile, not a cycle, since that sibling's own self-edge
    # targets itself and stops there. A primal-keyed guard would conflate that with genuine
    # in-progress-ness and bail incorrectly. See the comment on `build_reverse_fwds_ir` for the full
    # reasoning.
    #
    # Direct self-recursion (callee primal mi == current build's own) is a separate question,
    # answered locally and ctx-independently by
    # `reverse_fwds_recursive_ci`/`reverse_pullback_recursive_ci` from an explicitly-passed
    # `primal_mi`, not by consulting this field. This guard is only reached when the recursive edge's
    # target carrier differs from the one currently being built (the `Ctx{<:Tape}`-sibling case
    # above); a literal self-edge resolves to a static self-`:invoke` without recursing into the
    # builder at all. Mode-agnostic field (harmless, always empty, for `Forward`).
    ...
    # instance), so a later interpreter can hit an already-cached bailed carrier without re-running
    # `build_contextual_ir` — a fresh per-instance dict would have nothing in it, defeating the point
...
#
#   * `finishinfer!` — the pipeline function that *supplies* the `CodeInstance` return type. We build
#     the transformed IR here and set `me.bestguess` to its return type, so the ordinary
...
# When a mode's transform hits a construct it can't handle (control flow, unsupported builtins, …) it
# returns `nothing`: we leave `me.bestguess`/the cache untouched, the carrier stub's throwing body
# flows through the pipeline normally, and the compiled CI raises when invoked — a graceful bail.
# ---------------------------------------------------------------------------
...
function CC.finishinfer!(me::CC.InferenceState, interp::ADInterpreter, cycleid::Int,
    ...
    if ir !== nothing
        # `build_contextual_ir` builds `ir` via the 6-arg `CC.IRCode(...)` constructor, which defaults
        # `valid_worlds` to the unbounded sentinel `WorldRange(0, typemax(UInt))`. That sentinel fails
        # `abstract_eval_globalref_type`'s partition-coverage check
        # (`Compiler/src/abstractinterpretation.jl`) for any `GlobalRef` whose binding partition
        # doesn't itself span the full sentinel range — e.g. `Base.add_float` (an *imported* binding,
        # pulled in when inlining primal library code like `increment!!`), whose partition starts at a
        # finite world — so `argextype`/the IR pretty-printer mislabels those calls "dynamic" even
        # though they're ordinary builtin/intrinsic calls. Fix by giving `ir` a real world range:
        # `interp.world` onward, since every binding this IR references already resolved successfully
        # at that world.
        ...
        # calls — see `build_contextual_ir`/`build_dual_ir` in `forward_interp.jl`). These must land on
        # *this* CodeInstance (`me.linfo`'s own compile), not merely on some caller of it: `finish!` on
        # THIS `InferenceState` is what calls `store_backedges` against `me.linfo`'s CodeInstance
        # below, and a `@generated` caller's own cached CodeInstance for a call to `me.linfo` is only
        # re-examined for staleness by looking `me.linfo`'s cache up again — so if `me.linfo`'s
        # CodeInstance were never itself invalidated, a caller regenerating around it would just keep
        # reusing the same stale result. Setting `me.src.edges` here, before delegating to the generic
        # `finishinfer!` (which internally calls `compute_edges!(me)`, reading this field), is the
        # standard, documented way a generator-produced `CodeInfo` declares extra edges
        # (`compute_edges!` in `typeinfer.jl`).
...
# Replaces the optimization result with the transformed IR built in `finishinfer!`, then runs the
# ordinary IPO-safe optimization passes on it (inlining, SROA, ADCE, …) so the synthetic
