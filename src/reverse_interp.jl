...
#     `ADInterpreter{Reverse}` (ordinary code compiles under a `NativeInterpreter`, which never calls
...
# `P` is usually a plain `Type` (`pir.stmts[i][:type]`), but a statement whose result the primal's own
# const-prop narrowed (e.g. a `Core.memorynew` call with a literal length) can carry a
# `Core.PartialStruct`/`Const` lattice element instead — widen it first, since `tangent_type` is only
# defined on `Type`s. Mirrors `tt` in `forward_interp.jl` (`dualize_to_ircode`).
...
# `ADInterpreter{Forward}` — this must reuse the caller's own `interp` instead: `reverse_fwds_impl`
# specializations only get transformed via `build_contextual_ir` when compiled under an
# `ADInterpreter`, so a bare `NativeInterpreter` would just compile the untransformed stub
    ...
    if hand !== nothing
        ...
        if pmatch !== nothing && isa(pmatch.method, Method)
            ...
            if callee_primal_mi === current_primal_mi
                ...
                # `interp.in_progress` would (correctly) flag that as a genuine cycle and bail the
                # whole thing.
...
#    `code_cache(state.interp)`, keyed by `CC.cache_owner(::ADInterpreter{Reverse}) === Reverse()`; a
...
#    `src_inlining_policy(::ADInterpreter{Reverse}, ...)` only blocks carrier and hand-rule MIs, so
