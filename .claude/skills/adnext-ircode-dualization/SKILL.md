...
| any intrinsic call | `wrapper = translate(Val(f))`, then rewrite to the wrapper function and route through `frule_split!` — the wrapper's `frule` in `src/intrinsics.jl` supplies the rule. **Differentiable** intrinsics (`add_float`/`sub_float`/`neg_float`/`mul_float`/`div_float`, `_fast` variants) have hand-written rules (product/quotient rule for `mul`/`div`); **non-differentiable** ones (comparisons, integer/bit ops) are registered via `@inactive_intrinsic`, whose auto-generated `frule` computes the primal and gives it a zero tangent. Both survive as an `invoke frule(...)`, like `sin`/`cos`. Registered families: the differentiable float ops (`add/sub/neg/mul/div_float`, `sqrt_llvm`, `abs_float`, `max/min_float`, `fma_float`, `muladd_float`, `copysign_float`, `fpext`/`fptrunc`) and the inactive ones (comparisons, integer/bit ops, rounding, int↔float conversions). Handling is **explicit**: `translate` has no identity fallback, so an *unregistered* intrinsic throws — the engine catches it and bails gracefully with a located reason (`"unsupported intrinsic \`bitcast\` at %i: …"`), so a missing derivative surfaces loudly instead of a silent wrong zero. |
